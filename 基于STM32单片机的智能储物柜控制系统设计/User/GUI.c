 ...
 *=====================================================================
	V1.0		 3/28/2024	  Lee			形参中的u8型指针改为char型  Show_Str()函数中的判断语句再次强转成u8型
 ***********************************************************************/
...
void LCD_ShowNum_back_black(u16 x,u16 y,u32 num,u8 len,u8 size)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
...
void LCD_ShowNum_back_blue(u16 x,u16 y,u32 num,u8 len,u8 size)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,POINT_COLOR,BLUE,' ',size,0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,POINT_COLOR,BLUE,temp+'0',size,0); 
	}
} 
/*****************************************************************************
