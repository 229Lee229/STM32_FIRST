...
// 重定义fputc函数			此函数已在serial_jdy31.c中定义
...
void My_USART_Init(USART_TypeDef* USARTx, u32 bound){
	if(USARTx == USART1){
#if EN_USART1_RX		/* 如果使能了串口1 */					/* ENABLE(1)  |  DISENABLE(0) Serial Port 1 */
	Usart1_Init(bound);	
#endif	
	}
	if(USARTx == USART2){
#if EN_USART2_RX		/* 如果使能了串口2 */
	Usart2_Init(bound);
#endif
	}
	if(USARTx == USART3){
#if EN_USART3_RX
	Usart3_Init(bound);	/* 如果使能了串口3 */
#endif		
	}
